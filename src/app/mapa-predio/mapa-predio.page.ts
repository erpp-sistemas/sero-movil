...
@Component({
  selector: 'app-mapa-predio',
  templateUrl: './mapa-predio.page.html',
  styleUrls: ['./mapa-predio.page.scss'],
})
export class MapaPredioPage implements OnInit {
  ...
  async getAccountInfo(id_plaza) {//realiza la carga de informacion que existe en la base interna sqlite
    ...
    this.markersArrayInfo = await this.service.getDataVisitPositionPredio(id_plaza);
  ...
  async find() {
    if (this.accountString == "") {
      ...
      this.result = await this.service.getDataVisitPositionByAccountPredio(this.accountString);
